...
def assert_is_test_db() -> None:
    assert cccat.config.MONGODB_URL.endswith("_test")
...
def mock_config(monkeypatch: Any) -> None:
    ...
    """
    monkeypatch.setattr("cccat.config.ENABLE_FOO", "true")
    monkeypatch.setattr("cccat.config.ENABLE_BAR", "false")
...
def prevent_actual_requests(monkeypatch: Any) -> None:
    def mock_request(*args: Any, **kwargs: Any) -> None:
        raise Exception(
            ...
            "See cccat.tests.conftest.prevent_actual_requests for more information."
        )
