...
autoformat() {
  ...
  echo '---- format (isort) ----'
  poetry run isort cccat tests $ISORT_FLAG
}

check_style() {
  echo '---- style ----'
  poetry run flake8 cccat tests
}

check_type() {
  echo '---- type ----'
  poetry run mypy cccat tests
}

check_unit() {
  # default to coverage mode
  PYTEST_FLAGS='--cov=cccat --ignore=cccat --cov-report=term-missing --cov-report=xml'
